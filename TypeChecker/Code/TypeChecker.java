   ...
   //
	...
	public void printdump(){
		   if(debug){
			   ...
			   System.out.println(vars_in_scope.toString()); 
			   System.out.println();
   ...
   public R visit(ClassExtendsDeclaration n, A argu) {
	   if(!first_pass_done){
		   ...
		   if(mclass.equals(n.f3.f0.tokenImage))
			   error("Same as main class:"+n.f3.f0.tokenImage);
	   }
		  
	   if(first_pass_done){
//		   if(!vars_in_scope.get(":global").contains(n.f3.f0.tokenImage)){
...
//      n.f2.accept(this, argu);
      ...
      if (first_pass_done) {
    	  ...
    	  if (parent.containsKey(scope_stack.get(1))) {
    		  ...
    		  // Check if fun exists in parents
    		  String does_exists = getTypeForScope(fn_name, "::"+pclass);
    		  
    		  // If exists
    	      if(does_exists != ""){
    	    	  // If return type mismatch, error()
    	          if(mtype != does_exists)
    	        	  error("Return type mismatch in pclass:"+mtype+","+does_exists);

    	          String fn_class = getClassForScope(fn_name, "::" + pclass);
    	          String fn_params = fn_param.get(fn_class + ":" + fn_name);

    	          // If argument mismatch, error()
    	          if(!fn_params.equals(fn_param.get(full_scope() + ":" + fn_name))) {
    	        	  error("Argument mismatch:"+fn_params+", "+
    	        			  fn_param.get(full_scope() + ":" + fn_name));
    	          }
    	      }
    	  }
      }
...
public R visit(OrExpression n, A argu) {
   ...
   public R visit(MessageSend n, A argu) {
      ...
      if(first_pass_done){
    	  ...
    	  _ret = getTypeForScope(fn_name, "::"+cname);
    	  if(_ret == ""){
...
//      n.f2.accept(this, argu);
      ...
      if(first_pass_done){
    	  ...
    	  String real_class_name = getClassForScope(fn_name, "::"+cname);
    	  if(real_class_name == "")
              error("Class not found in scope in Msgsend:"+cname);
          if(etype == null)
    		  etype = "";
    	  
    	  String[] params 		= fn_param.get(real_class_name + ":" + fn_name).split(",");
